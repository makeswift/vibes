      ...
      <div className="@sm:gap-x-8 @sm:gap-y-8 @3xl:gap-x-10 @3xl:gap-y-10 grid w-full grid-cols-12 gap-x-0 gap-y-6">
        {items?.slice(0, Math.min(page, items.length)).map((item, index) => (
          <Link
            ...
            key={index}
            className={clsx(
              '@sm:rounded-3xl bg-muted-background/50 group z-0 overflow-hidden rounded-2xl text-foreground ring-1 ring-foreground/20 transition duration-500 hover:bg-primary/10 hover:ring-primary/50',
              index === 0
