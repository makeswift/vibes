...
type Props = {
  ...
  options: Array<
    { label: string; value: string } | { label: string; action: () => void | Promise<void> }
  >;
  className?: string;
  errors?: string[];
                ...
                if ('action' in option) {
                  return (
                    <form
                      ...
                      className="flex w-full cursor-default select-none rounded-xl px-3 py-2 text-sm font-medium text-contrast-400 outline-none transition-colors hover:bg-contrast-100 hover:text-foreground focus-visible:bg-contrast-100 focus-visible:text-foreground data-[state=checked]:text-foreground @4xl:text-base"
                      key={`option-action-${option.label}`}
                    >
                      <button className="flex-1 cursor-default text-left" type="submit">
                        {option.label}
                      </button>
                    </form>
                  );
